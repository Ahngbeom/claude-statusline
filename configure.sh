...
CATEGORY_DEFS=(
  ...
  "Line 1 fields (git, cc version, output style, memory, session cmd)|STATUSLINE_SHOW_GIT STATUSLINE_SHOW_GIT_STATUS STATUSLINE_SHOW_CC_VERSION STATUSLINE_SHOW_OUTPUT_STYLE STATUSLINE_SHOW_MEM STATUSLINE_SHOW_SESSION_CMD STATUSLINE_SESSION_CMD"
  ...
  "Line 1 colors (dir, model, git, cc version, output style, session cmd)|STATUSLINE_COLOR_DIR STATUSLINE_COLOR_MODEL STATUSLINE_COLOR_GIT STATUSLINE_COLOR_CC_VERSION STATUSLINE_COLOR_OUTPUT_STYLE STATUSLINE_COLOR_SESSION_CMD"
  ...
  "Icons|STATUSLINE_ICON_DIR STATUSLINE_ICON_CONTEXT STATUSLINE_ICON_COST STATUSLINE_ICON_CACHE STATUSLINE_ICON_MEM STATUSLINE_ICON_SESSION_CMD"
)
...
#   surface to validate against. STATUSLINE_SESSION_CMD is `text` for a
#   different reason: it is never rendered as-is at all, only fed through
#   statusline.sh's _format_session_cmd flag whitelist, which drops anything
#   outside [A-Za-z0-9._-] before it can reach the screen.
...
_key_type() {
  case "$1" in
    ...
    STATUSLINE_COLOR_SESSION_OK|STATUSLINE_COLOR_SESSION_WARN|STATUSLINE_COLOR_SESSION_CRIT|\
    STATUSLINE_COLOR_MEM_OK|STATUSLINE_COLOR_MEM_WARN|STATUSLINE_COLOR_MEM_CRIT|\
    STATUSLINE_COLOR_SESSION_CMD) echo color256 ;;
    STATUSLINE_ICON_DIR|STATUSLINE_ICON_CONTEXT|STATUSLINE_ICON_COST|STATUSLINE_ICON_CACHE|STATUSLINE_ICON_MEM|\
    STATUSLINE_ICON_SESSION_CMD|STATUSLINE_SESSION_CMD|\
...
_key_default() {
  case "$1" in
    ...
    STATUSLINE_SESSION_CMD) echo "(auto-detected)" ;;
    ...
    STATUSLINE_COLOR_SESSION_CMD) echo 245 ;;
    ...
    STATUSLINE_ICON_SESSION_CMD) echo "⌘" ;;
...
_key_description() {
  case "$1" in
    ...
    STATUSLINE_SHOW_SESSION_CMD) echo "Line 1: CLI flags this session was started with" ;;
    STATUSLINE_SESSION_CMD) echo "Override the detected launch command (e.g. 'claude -c')" ;;
    ...
    STATUSLINE_COLOR_SESSION_CMD) echo "256-color code: Line 1 session launch flags" ;;
    ...
    STATUSLINE_ICON_SESSION_CMD) echo "Icon: Line 1 session launch flags prefix" ;;
