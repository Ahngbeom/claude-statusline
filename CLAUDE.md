...
| 헬퍼 함수 | ~50-102 | `to_epoch()`, `progress_bar()`, `format_tokens()` 등 순수 bash 구현 |
| 캐싱 레이어 | ~104-163 | ccusage 결과 캐시, 4개 명령 병렬 실행으로 백그라운드 갱신 |
| 입력 파싱 | ~165-185 | 단일 jq 호출, Unit Separator(0x1f) 구분자 |
| 컨텍스트 계산 | ~193-239 | 모델별 컨텍스트 윈도우 매핑, 사용률 기반 동적 색상 |
| ccusage 통합 | ~241-328 | 일/주/월 통계, 세션 시간, 캐시 히트율 |
| 렌더링 | ~340-443 | 3줄 출력 조립 |
